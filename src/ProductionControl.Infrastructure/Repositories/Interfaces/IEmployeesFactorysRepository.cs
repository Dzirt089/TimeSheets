...
namespace ProductionControl.Infrastructure.Repositories.Interfaces
{
	public interface IEmployeesFactorysRepository
	{
		...
		/// </summary>
		Task SetNamesDepartmentAsync(CancellationToken token = default);
		...
		/// </summary>
		Task SetTotalWorksDaysAsync(ShiftData shiftData, CancellationToken token = default);

		/// <summary>
		/// Очищаем данные на сегодня, перед новыми заказами обедов
		/// </summary>
		Task CleareDataForFormulateReportForLunchEveryDayDbAsync(CancellationToken token = default);
		...
		/// </summary>
		Task ClearLastDeport(DataForClearLastDeport dataForClear, CancellationToken token = default);
